...
### 2. Ineternet Layer - 인터넷 계층

- OSI 7 Layer 에서 네트워크 계층과 유사한 역할을 한다.
- 데이터그램(IP 프로토콜에서 다루는 패킷 데이터)를 정확한 수신지로 전송하는 역할을 한다.
- 일반적으로 견고함의 원칙을 기준으로 설계를 한다고 한다.
  - 견고함의 원칙 : 수신 대상을 자유롭게 하고 송신 대상을 보수적으로 하는 것



### 3. Transport Layer - 전송 계층

- OSI 7 Layer 에서 전송 계층에 해당한다.
- 네트워크 양단의 송 수신 호스트 사이에서 신뢰성있는 전송기능을 제공한다.
  - TCP, UDP 프로토콜
...
- OSI 7 Layer 에서 세션 계층, 표현계층, 애플리케이션 계층에 해당한다.
- 응용 프로그램들이 네트워크 서비스, 메일 서비스, 웹 서비스 등을 할 수 있도록 표준적인 인터페이스를 제공한다.
- 웹 브라우저나 인터넷 메일 프로그램이 응용 계층에 해당한다.



## 결론

컴퓨터 간에 통신을 하는데는 절차가 필요하다.

OSI 표준 모델과 TCP/IP 는 컴퓨터 간에 통신을 하는 절차를 보여주는 것 같다.

그 과정을 크게 나누면

1. 전기적 신호를 컴퓨터가 이해할 수 있는 형식으로 바꾸거나 컴퓨터 언어를 전기적 신호로 바꾸는 과정
2. 어디로 보낼 지 정하는 과정
3. 어떻게 보낼 지 정하는 과정, 데이터가 다 전달되었는지 혹은 순서에 상관없이 받아올 건지에 대한 과정
   추가적으로는 연결이 되었는지 안되었는지를 고려하는 과정
4. 받아와서 처리한 데이터를 실제로 사용하는 과정

각 절차마다 담당하는 역할이 다르고 분리되어 있기 때문에 ( 물론 TCP / IP 는 분리가 모호하다는 말이 있지만 ) 문제해결을 하는데 참고해서 진행하는 것이 필요하다.
...
- https://ko.wikipedia.org/wiki/%EC%9D%B8%ED%84%B0%EB%84%B7_%EA%B3%84%EC%B8%B5
